...
def plan_T_E_auto(category_info: Dict[str, List[str]],
                  study_mode: str = "layout",
                  max_active_per_row: Optional[int] = None) -> Tuple[int, int, Dict[str, int], float, int]:
    """Plan T and E automatically with improved ratio-aware logic."""
    cats = list(category_info.keys())
    q = {c: len(category_info[c]) for c in cats}
    C = len(cats)
    M = sum(q.values())
    ...
    # Helper: maximum feasible E at a given T
    def E_upper_at_T(T_try: int) -> int:
        # Upper bound from a fast algebraic approximation (may be off by 1 due to ceil()).
        approx_ratio = min(int(math.floor(T_try / (q[c] + ABSENCE_RATIO))) for c in cats)

        # Per-row active cap: total 1s = M*E <= T * rowcap
        rowcap = (max_active_per_row if (study_mode == "grid" and max_active_per_row is not None) else C)
        approx_rowcap = int(math.floor(T_try * rowcap / M))

        E = min(approx_ratio, approx_rowcap)

        # Tighten to the exact absence constraint:
        #   T_try >= q[c]*E + ceil(ABSENCE_RATIO * E)  for all categories c
        # This avoids rare off-by-one errors when ABSENCE_RATIO is fractional.
        def absence_ok(E_test: int) -> bool:
            if E_test < 0:
                return False
            reqA = int(math.ceil(ABSENCE_RATIO * E_test))
            for c in cats:
                if T_try < q[c] * E_test + reqA:
                    return False
            return True

        while E > 0 and not absence_ok(E):
            E -= 1

        return E

    # NEW: minimum E needed so row-mix feasibility is possible:
    # plan_row_mix requires total_ones = M*E to satisfy: T*MIN_ACTIVE_PER_ROW <= total_ones <= T*max_k
    # Here we enforce the lower bound up-front: M*E >= T*MIN_ACTIVE_PER_ROW.
    def E_min_for_rowmix(T_try: int) -> int:
        return int(math.ceil((T_try * MIN_ACTIVE_PER_ROW) / M))
    ...
    while True:
        if T > max(cap - slack, 0):
            if T > cap:
                raise RuntimeError(
                    "Infeasible: T exceeds visible capacity. "
                    "Add elements/categories or relax ABSENCE_RATIO/T_RATIO."
                )
            slack = 0
            
        E_up = E_upper_at_T(T)

        # NEW: Require enough E to satisfy BOTH:
        #  (a) your configured minimum exposures, and
        #  (b) row-mix feasibility given MIN_ACTIVE_PER_ROW
        E_needed = max(PER_ELEM_EXPOSURES, E_min_for_rowmix(T))

        if E_up >= E_needed:
            # Choose E_up to maximize exposures while remaining feasible.
            E = E_up
            break
